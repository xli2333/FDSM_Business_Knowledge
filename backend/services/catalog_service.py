...
from backend.services.content_operations_service import (
    get_trending_config,
    get_trending_sql_parts,
    list_home_content_slots,
)
...
def _fetch_published_editorial_assets(connection, article_id: int) -> dict[str, str | None]:
    row = connection.execute(
        """
        SELECT
            COALESCE(published_final_html, final_html, html_web, html_wechat) AS final_html,
            COALESCE(published_summary_html, summary_html) AS summary_html,
            COALESCE(summary_markdown, excerpt) AS summary_text
        FROM editorial_articles
    ...
    if row is None:
        return {"final_html": None, "summary_html": None, "summary_text": None}
    return {
        "final_html": str(row["final_html"] or "").strip() or None,
        "summary_html": str(row["summary_html"] or "").strip() or None,
        "summary_text": str(row["summary_text"] or "").strip() or None,
    }


def _resolve_article_summary_html(*, summary: str, stored_html: str | None) -> str | None:
    normalized_summary = _normalize_catalog_summary_text(summary)
    html = str(stored_html or "").strip()
...
def _normalize_catalog_summary_text(summary: str) -> str:
    source = str(summary or "").strip()
    if not source:
        return ""

    normalized = normalize_summary_display_markdown(source, "zh").strip()
    hybrid_candidate = ai_service.normalize_editorial_summary_output(source).strip()
    ...
    if source_has_list and hybrid_has_list and hybrid_length >= ai_service.EDITORIAL_SUMMARY_MIN_CHARS:
...
def _query_trending_rows(connection, *, window: str, limit: int, offset: int = 0):
...
def list_trending_articles(
    *,
    ...
    with connection_scope() as connection:
        rows = _query_trending_rows(connection, window=window, limit=safe_limit, offset=safe_offset)
        items = _serialize_articles(connection, rows, language=language)
        return _take_language_ready_articles(items, safe_limit, language)
...
def get_article_detail(
    ...
    with connection_scope() as connection:
        ...
        editorial_assets = _fetch_published_editorial_assets(connection, article_id)
        preferred_content = row["content"]
        summary_text = row["excerpt"] or row["main_topic"] or ""
        if editorial_assets["summary_text"]:
            summary_text = editorial_assets["summary_text"]
        ...
        if (
            not editorial_assets["summary_text"]
            and ai_row is not None
            and ai_row["summary_status"] == "completed"
            and str(ai_row["summary_zh"] or "").strip()
        ):
        ...
        summary_text = _normalize_catalog_summary_text(summary_text)
        summary_html = _resolve_article_summary_html(
            summary=summary_text,
            stored_html=editorial_assets["summary_html"] or (ai_row["summary_html_zh"] if ai_row is not None else None),
        )
        visible_html = _resolve_article_html(
            ...
            summary=summary_text,
            source_url=row["link"],
            stored_html=None if access["locked"] else (editorial_assets["final_html"] or (ai_row["html_wechat_zh"] if ai_row is not None else None)),
        )
...
def get_home_feed(language: str = "zh") -> dict:
    ...
    def _ordered_serialized_articles(connection, article_ids: list[int]) -> list[dict]:
        if not article_ids:
            return []
        placeholders = ",".join("?" for _ in article_ids)
        rows = connection.execute(
        ...
        serialized = _serialize_articles(connection, rows, language=language)
        item_map = {item["id"]: item for item in serialized}
        return [item_map[article_id] for article_id in article_ids if article_id in item_map]

    def _ordered_localized_tags(connection, slugs: list[str]) -> list[dict]:
        if not slugs:
            return []
        ...
        row_map = {row["slug"]: dict(row) for row in rows}
        payload = []
        for slug in slugs:
            row = row_map.get(slug)
            if not row:
                continue
            localized = localize_tag_payload(row, language=language)
            if localized:
                payload.append(localized)
        return payload

    topic_payload_map = {item["slug"]: item for item in list_topics(language=language)}
    with connection_scope() as connection:
        hero_ids = [int(item["entity_id"]) for item in home_slots.get("hero", []) if item.get("entity_id") is not None]
        editor_ids = [int(item["entity_id"]) for item in home_slots.get("editors_picks", []) if item.get("entity_id") is not None]
        ...
        latest_rows = connection.execute(
            ...
            LIMIT ?
            """,
            (24 if language == "en" else 12,),
        ).fetchall()
        trending_rows = _query_trending_rows(connection, window=trending_window, limit=18 if language == "en" else 6)

        hero_item = None
        hero_candidates = _ordered_serialized_articles(connection, hero_ids[:1])
        if hero_candidates:
            candidate = hero_candidates[0]
            if language != "en" or english_article_ready(candidate):
                hero_item = candidate

        serialized_picks = _take_language_ready_articles(_ordered_serialized_articles(connection, editor_ids), 6, language)
        serialized_trending = _take_language_ready_articles(_serialize_articles(connection, trending_rows, language=language), 6, language)
        serialized_latest = _take_language_ready_articles(_serialize_articles(connection, latest_rows, language=language), 12, language)
        ...
        column_previews = []
        if column_slugs:
            placeholders = ",".join("?" for _ in column_slugs)
            ...
            column_map = {row["slug"]: row for row in column_rows}
            for column_slug in column_slugs:
                column = column_map.get(column_slug)
                if column is None:
                    continue
                rows = connection.execute(
                    """
        ...
        quick_tags = _ordered_localized_tags(connection, quick_tag_slugs)
        topic_starters = [topic_payload_map[slug] for slug in topic_starter_slugs if slug in topic_payload_map]
        topic_square = [topic_payload_map[slug] for slug in topic_square_slugs if slug in topic_payload_map]
        return {
            "hero": hero_item,
            "editors_picks": serialized_picks,
            "trending": serialized_trending,
            "trending_window": trending_window,
            "trending_windows": ["day", "week", "month"],
            "column_previews": column_previews,
            "latest": serialized_latest,
            "hot_tags": quick_tags,
            "quick_tags": quick_tags,
            "topic_starters": topic_starters,
            "topics": topic_square,
