...
from backend.config import DEV_AUTH_ENABLED, SUPABASE_ANON_KEY, SUPABASE_AUTH_TIMEOUT_SECONDS, SUPABASE_URL
...
def get_auth_status_payload(
    ...
    *,
    ...
    return {
        "enabled": is_supabase_auth_enabled() or DEV_AUTH_ENABLED,
        "authenticated": bool(user),
        "user": {"id": user["id"], "email": user.get("email")} if user else None,
        "auth_mode": "password" if DEV_AUTH_ENABLED else "email_otp",
