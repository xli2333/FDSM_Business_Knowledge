...
def test_serialize_media_row_returns_authenticated_stream_url_for_accessible_local_audio():
    row = {
        "id": 1,
        "slug": "sample-audio",
        "kind": "audio",
        ...
        "media_url": "/audio-files/sample.mp3",
        "preview_url": "",
        "source_url": "/audio-files/sample.mp3",
        "body_markdown": "Long body",
        "transcript_markdown": "Long transcript",
        "chapter_payload_json": "[]",
    }

    payload = _serialize_media_row(row, _membership("paid_member", can_access_member=True, can_access_paid=True))

    assert payload["accessible"] is True
    assert payload["media_url"] == "/api/media/audio/sample-audio/stream"
    assert payload["source_url"] == "/api/media/audio/sample-audio/stream"
