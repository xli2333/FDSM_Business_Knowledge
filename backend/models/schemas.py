...
class HomeFeedResponse(BaseModel):
    ...
    trending_windows: list[str] = Field(default_factory=lambda: ["day", "week", "month"])
    ...
    quick_tags: list[TagSummary] = Field(default_factory=list)
    topic_starters: list[TopicSummary] = Field(default_factory=list)
    topics: list[TopicSummary] = Field(default_factory=list)
    topic_square: list[TopicSummary] = Field(default_factory=list)
...
class MediaItemSummary(BaseModel):
    ...
    preview_duration_seconds: int = 0
...
class AdminContentSection(BaseModel):
    slot_key: str
    entity_type: str
    max_items: int = 0
...
class AdminContentSectionUpdateRequest(BaseModel):
    items: list[AdminContentEntity] = Field(default_factory=list)
...
class AdminTrendingConfig(BaseModel):
    ...
    like_weight: float = 4.0
    bookmark_weight: float = 6.0
    updated_at: str | None = None


class AdminTrendingConfigUpdateRequest(BaseModel):
    default_window: str = "week"
    view_weight: float = 1.0
    like_weight: float = 4.0
    bookmark_weight: float = 6.0
...
class EditorialRenderSnapshot(BaseModel):
...
class EditorialArticleBase(BaseModel):
    ...
    primary_column_manual: bool = False
    article_type: str | None = None
    main_topic: str | None = None
    access_level: str = "public"
    ...
    tags: list[EditorialTagSuggestion] | None = None
    selected_topic_ids: list[int] | None = None


class EditorialArticleCreate(EditorialArticleBase):
    slug: str | None = None
    summary_editor_document: dict[str, Any] | None = None
...
class EditorialArticleUpdate(BaseModel):
    ...
    cover_image_url: str | None = None
    primary_column_slug: str | None = None
    primary_column_manual: bool | None = None
    article_type: str | None = None
    main_topic: str | None = None
...
class EditorialArticleSummary(BaseModel):
    ...
    updated_at: str
    primary_column_slug: str | None = None
    primary_column_ai_slug: str | None = None
    primary_column_manual: bool = False
    article_type: str | None = None
    main_topic: str | None = None
    access_level: str = "public"
    ...
    ai_tags: list[EditorialTagSuggestion] = Field(default_factory=list)
    removed_tags: list[EditorialTagSuggestion] = Field(default_factory=list)
    has_unpublished_changes: bool = False
    draft_box_state: str = "active"
    is_reopened_from_published: bool = False
    publish_validation: list[EditorialValidationIssue] = Field(default_factory=list)
...
class EditorialArticleDetail(EditorialArticleSummary):
    ...
    summary_markdown: str | None = None
    summary_html: str | None = None
    published_summary_html: str | None = None
    summary_editor_document: dict[str, Any] = Field(default_factory=dict)
    ...
    editor_source: str | None = None
    editor_updated_at: str | None = None
    ...
    topic_candidates: list[AdminContentEntity] = Field(default_factory=list)
    created_at: str
...
class EditorialDeleteResponse(BaseModel):
    editorial_id: int
    deleted: bool = True
