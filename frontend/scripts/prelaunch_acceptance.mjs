...
async function runGuestDesktopFlow(browser, articleIds) {
  ...
  results.push(
    await runStep(page, diagnostics, cursor, 'guest-desktop', 'audio', async () => {
      const audioVisible = await ensureVisible(page.getByText(/Member audio and ongoing listening|Member audio|Audio|会员音频与持续收听|会员音频|音频/))
  ...
  results.push(
    await runStep(page, diagnostics, cursor, 'guest-desktop', 'video', async () => {
      const videoVisible = await ensureVisible(page.getByText(/Video Hub|视频栏目/))
