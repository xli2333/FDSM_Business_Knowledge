...
function previewHtmlLooksMarkdownLiteral(value) {
  ...
  if (/<\s*(html|body|div|section|article|table|thead|tbody|tr|td|th|h1|h2|h3|p|ul|ol|li|blockquote)\b/i.test(html)) {
    return false
  }
...
function ArticlePage() {
  ...
  const handleReopenDraft = async () => {
    ...
    setStatusHint('')
    try {
      const payload = await reopenEditorialSourceArticle(article.id)
      navigate(`/editorial?editorial_id=${payload.id}&reopened=1`)
    } catch (error) {
  ...
  const visibleSummary = translatedActive ? translation.summary : article.summary
  const visibleSummaryHtml = translatedActive ? translation?.summary_html : article.summary_html
  ...
  const summaryFallback = translatedActive ? '...' : isEnglish ? 'Summary unavailable.' : '摘要暂不可用。'
