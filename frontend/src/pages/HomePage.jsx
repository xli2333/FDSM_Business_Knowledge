...
import { fetchHomeFeed, fetchLatestArticles, resolveApiUrl } from '../api/index.js'
...
function HomePage() {
  ...
  const heroQuickTags = (data?.hot_tags || []).slice(0, 6)
  const heroQuickTopics = (data?.topics || []).slice(0, 2)
    ...
    ? membership?.tier === 'admin'
      ...
      : membership?.tier === 'paid_member'
        ? {
            ...
            title: isEnglish ? 'Start with benefits, full articles, and audio' : '从权益、完整正文和音频开始',
            body: isEnglish
              ? 'Paid members land where their subscription matters most: full-text reading, member audio, and account benefits.'
              : '付费会员会优先看到完整正文、会员音频和当前订阅权益。',
            ctaLabel: isEnglish ? 'Open membership' : '进入会员空间',
            ctaPath: '/membership',
          }
        : {
            kicker: isEnglish ? 'For free members' : '免费会员入口',
            title: isEnglish ? 'Keep your library, follows, and upgrade options in one place' : '把资料库、关注和升级选择放在一个页面里',
            body: isEnglish
              ? 'Free members can continue from bookmarks, likes, reading history, and followed topics, then upgrade when deeper access is needed.'
              : '免费会员可以从收藏、点赞、阅读历史和关注话题继续阅读，并在需要更深层权限时升级。',
            ctaLabel: isEnglish ? 'Open my library' : '进入我的资料库',
            ctaPath: '/me',
          }
    : {
        kicker: isEnglish ? 'For guests' : '访客入口',
        title: isEnglish ? 'Start with public reading' : '先从公开阅读开始',
        body: isEnglish
          ? 'Guests can search, read public articles, and explore topics before deciding whether to sign in or upgrade.'
          : '访客可以先搜索、阅读公开文章和浏览专题，再决定是否登录或升级。',
